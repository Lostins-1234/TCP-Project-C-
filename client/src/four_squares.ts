 ...
 */
export function decomposeFourSquares(val: bigint): [bigint, bigint, bigint, bigint] {
    if (val < 0n) throw new Error("Negative value");
    if (val === 0n) return [0n, 0n, 0n, 0n];

    const result = findSquares(val, 4);
    if (!result) throw new Error("Could not decompose");
...
function findSquares(target: bigint, depth: number): bigint[] | null {
    if (depth === 1) {
...
function sqrtBigInt(value: bigint): bigint {
    if (value < 0n) throw new Error("negative number is not supported");
